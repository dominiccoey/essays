...
$(q^{H}_1,\ldots,q^{H}_n)$.
  - Production is [O-ring](https://en.wikipedia.org/wiki/O-ring_theory_of_economic_development): we simply multiply all the relevant task-specific productivities together. Interpret _scaffolding_ as a costly investment to isolate AI contributions to a specific subset of tasks, as opposed to having either a human or “off-the-shelf” AI do everything. If scaffolding is sufficiently cheap, we choose the best option task-by-task, and achieve maximal output $\prod_{i=1}^n \max \\{q^{H}_i, q^{AI}_i\\}$.
